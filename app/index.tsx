...
export default function HomeScreen() {
  ...
  function toggleSimilarFiltersPanel() {
    ...
    setSimilarFiltersOpen((isOpen) => {
      if (isOpen) setSpecialFilters((current) => current.filter((filter) => !similarFilters.includes(filter)));
      return !isOpen;
    });
  }
  ...
  async function submitAuth() {
    ...
    // способ входа, если пароль сейчас недоступен.
    if ((authMode === "signup" || authMode === "otp") && authStep === 1) {
  ...
  return (
    <SafeAreaView style={styles.page}>
      ...
      {authOpen && (
        <Modal transparent animationType="slide" onRequestClose={() => setAuthOpen(false)}>
          ...
          <View style={styles.authHeader}>
            <Text style={styles.authTitle}>{isSignedIn && !(authMode === "signup" && authStep === 3) ? "Твой профиль" : authMode === "signup" && authStep === 3 ? "Придумай пароль" : authMode === "signup" ? "Регистрация в ЗаНомером" : authMode === "otp" ? "Вход по коду" : "Вход в ЗаНомером"}</Text>
            <Pressable
          ...
          {isSignedIn && !(authMode === "signup" && authStep === 3) ? (
            ...
            <>
              <Text style={styles.authHint}>{supabase ? authMode === "signup" ? `Шаг ${authStep} из 3` : authMode === "otp" ? (authStep === 1 ? "Укажи почту — пришлём одноразовый код." : "Введи код из письма.") : "Введи почту и пароль от аккаунта" : "Укажи имя — оно будет видно в твоих объявлениях."}</Text>
              {supabase && (authMode === "signup" || authMode === "otp") && authStep === 1 && <TextInput value={authEmail} onChangeText={setAuthEmail} placeholder="Твоя почта" placeholderTextColor="#98A2B3" style={styles.authInput} autoCapitalize="none" keyboardType="email-address" />}
              {supabase && (authMode === "signup" || authMode === "otp") && authStep === 2 && <TextInput value={authCode} onChangeText={setAuthCode} placeholder="Код из письма" placeholderTextColor="#98A2B3" style={styles.authInput} keyboardType="number-pad" autoCapitalize="none" />}
              ...
              <View style={styles.authRow}>
                {supabase && authMode === "signin" && <Pressable onPress={() => { setAuthMode("signup"); setAuthStep(1); setAuthPassword(""); setAuthCode(""); setAuthMessage(""); }} style={styles.authSwitch}><Text style={styles.authSwitchText}>Создать аккаунт</Text></Pressable>}
                {supabase && authMode === "signin" && <Pressable onPress={() => { setAuthMode("otp"); setAuthStep(1); setAuthPassword(""); setAuthCode(""); setAuthMessage(""); }} style={styles.authSwitch}><Text style={styles.authSwitchText}>Войти по коду</Text></Pressable>}
                ...
                {supabase && authMode === "otp" && <Pressable onPress={() => { setAuthMode("signin"); setAuthStep(1); setAuthPassword(""); setAuthCode(""); setAuthMessage(""); }} style={styles.authSwitch}><Text style={styles.authSwitchText}>Войти с паролем</Text></Pressable>}
      ...
      {(!compactLayout || filterPanelOpen) && <View style={styles.filterControlPanel}>
        <View style={styles.filterControlGroup}>
          ...
          <View style={styles.quickFilters}>
            ...
            <Pressable onPress={toggleSimilarFiltersPanel} style={[styles.quickFilter, similarFiltersOpen && styles.quickFilterActive]}>
      ...
      {activeTab === "buy" && !catalogOnly && similarFiltersOpen && <View style={styles.listFilterPlacement}>
        ...
        <View style={[styles.detailsOverlay, styles.sellerProfileOverlay]}>
          ...
          <View style={styles.legalPanel}>
            ...
            <ScrollView style={styles.legalScroll} contentContainerStyle={styles.legalScrollContent} nestedScrollEnabled keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator>
              ...
              <Text style={styles.legalText}>Мы обрабатываем только данные, необходимые для работы аккаунта и оказания услуг: адрес электронной почты, имя профиля, сведения об объявлениях и технические данные устройства. Данные не продаются третьим лицам. Платёжные реквизиты карт сервис не хранит — их обрабатывает ПСБ. Для вопросов об обработке данных и безопасности можно обратиться по телефону ниже.</Text>
              ...
              <Text style={styles.legalFootnote}>Все разделы доступны по кнопке ⓘ в шапке сайта. Перед отправкой заявки ПСБ останется добавить фактический/почтовый адрес и рабочий e-mail для обращений покупателей.</Text>
            </ScrollView>
          </View>
        </View>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  savedSearchesTitle: { color: "#101828", fontSize: 14, fontWeight: "800", textAlign: "center" },
  ...
  trustBadge: { alignSelf: "flex-start", backgroundColor: "#E8F8F0", borderColor: "#BAE9D1", borderRadius: 10, borderWidth: 1, maxWidth: "100%", paddingHorizontal: 9, paddingVertical: 5 },
  ...
  heart: { alignItems: "center", backgroundColor: "#F7F5FF", borderColor: "#DDD8FF", borderRadius: 14, borderWidth: 1, height: 42, justifyContent: "center", marginLeft: 2, width: 46 },
  heartText: { color: "#98A2B3", fontSize: 30, lineHeight: 32 },
  heartActive: { color: "#E31B54", fontSize: 30, lineHeight: 32 },
